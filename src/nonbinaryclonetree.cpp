...
NonBinaryCloneTree::NonBinaryCloneTree()
  : BaseTree()
  , _l(_tree)
{
}

NonBinaryCloneTree::NonBinaryCloneTree(const NonBinaryCloneTree& other)
  : BaseTree(other)
  , _l(_tree)
{
...
NonBinaryCloneTree::NonBinaryCloneTree(const Digraph& T,
                                       Node root,
...
bool NonBinaryCloneTree::readLeafLabeling(std::istream& in)
{
  for (NodeIt u(_tree); u != lemon::INVALID; ++u)
  {
    _l[u] = "";
...
void NonBinaryCloneTree::writeLeafLabeling(std::ostream& out) const
{
  for (Node u : _leafSet)
  {
                                  ...
                                  const IntNodeMap& characterLabel) const
{
  ...
  for (NodeIt u(_tree); u != lemon::INVALID; ++u)
  {
    if (_isLeaf[u])
    {
      out << "\t\t" << _tree.id(u) << " [width=0.2,height=0.2,penwidth=3,colorscheme=set19,color="
          << colorMap.find(lPlus[u])->second
          << ",label=\"" << std::setprecision(2) << 2 * 100 * U[u] << "%\"]" << std::endl;
    }
  }
  out << "\t}" << std::endl;
  ...
  out << "\tinv -> "
      ...
      << colorMap.find(lPlus[_root])->second << ",label=\"" << characterLabel[_root] + 1
      << "\",fontsize=26]" << std::endl;
  ...
  for (ArcIt a(_tree); a != lemon::INVALID; ++a)
  {
    ...
    if (s_u == s_v)
    {
      out << " [fontsize=26,penwidth=3,colorscheme=set19,color=" << colorMap.find(s_u)->second;
    }
    else
    {
      out << " [fontsize=26,penwidth=3,colorscheme=set19,color=\"" << colorMap.find(s_u)->second << ";0.5:" << colorMap.find(s_v)->second << "\"";
    }
    if (characterLabel[_tree.target(a)] != -1)
    {
      out << ",label=\"" << characterLabel[_tree.target(a)] + 1 << "\"";
    }
    out << "]";
    out << std::endl;
  }
...
void NonBinaryCloneTree::writeDOT(std::ostream& out) const
{
  StringToIntMap colorMap = generateColorMap();
...
void NonBinaryCloneTree::mergeSameSampleSiblingLeaves()
{
  typedef std::map<std::string, NodeList> SampleToNodeList;
  char buf[1024];
