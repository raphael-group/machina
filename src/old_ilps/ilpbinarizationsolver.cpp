...
IlpBinarizationSolver::IlpBinarizationSolver(const CloneTree& T,
...
void IlpBinarizationSolver::run(const CloneTree& T,
                                const std::string& primary,
                                ...
                                const StringPairList& forcedComigrations)
{
  ...
  {
    snprintf(buf, 1024, "%s/ilp-%s-%s-binarized.ilp",
             outputDirectory.c_str(),
             primary.c_str(),
             MigrationGraph::getPatternString(pattern).c_str());
  ...
  std::cerr << G.getNrMigrations() << " migrations, "
    << G.getNrComigrations(solver.getCloneTree(), solver.lPlus()) << " comigrations and "
